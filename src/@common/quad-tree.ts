...
export class QuadTree<T extends Spatial> {
    ...
    constructor(public depth: number, public tl: Vector, public br: Vector, public parent: QuadTree<T> | null = null) {
        if (depth > 0) {
            if (depth % 2) {
    ...
    inBound(vector: Vector): boolean {
        return (this.tl.x < vector.x && vector.x < this.br.x) && 
            (this.tl.y < vector.y && vector.y < this.br.y)
    }

    shortestDistanceWithinHeuristic(vector: Vector, oerHeuristic: number): boolean {
        if (this.inBound(vector)) return true
        if (Math.abs(this.tl.x - vector.x) < oerHeuristic ||
    ...
    closestWithDst(el: T, oerHeuristic = Number.POSITIVE_INFINITY, canGoUp = true): [number,T] | null {
        if (!this.shortestDistanceWithinHeuristic(el.pos, oerHeuristic)) return null

        if (this.nodeCount > +this.inBound(el.pos) && canGoUp) return this.parent && this.parent.closestWithDst(el, oerHeuristic)

        if (this.children != null) {
            const a = this.children[0].closestWithDst(el,            oerHeuristic, false)
            const b = this.children[1].closestWithDst(el, a ? a[0] : oerHeuristic, false)
    ...
    pop(el: T): void {
        if (!this.inBound(el.pos)) { return this.parent && this.parent.pop(el) }
    ...
    clear():void {
        if(this.children) return void [this.children[0].clear(),this.children[1].clear()]
